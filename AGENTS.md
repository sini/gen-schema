...
| A kind whose name starts with `_` is still **constructed** — `_hidden.kind` reads `"_hidden"` — but it is **refused by name** on every *kind-derived* schema read; the pre-`98a5ec0` silent exclusion is retired. The refusal is per-read, at force time, not at declaration | `lib/entry-type.nix`, bindings `isInternalField` and `reservedKindNames`; schema with kinds `visible` and `_hidden` ⇒ `_hidden.kind` is `"_hidden"`, while **all six kind-derived reads** — `_kindNames`, `_topology`, `_refEdges`, `_edges`, `_roots`, `_leaves` — **throw** `gen-schema: kind name '_hidden' is reserved — names starting with '_' are internal use only (_kindNames, _topology, etc.)`. `_collectionKeys`, the seventh, still answers ⇒ `[ "methods" "parent" "validators" ]` — it derives from the `collections` argument, not from kind names. Control, same run: kind `hidden` ⇒ `_kindNames` `[ "hidden" "visible" ]`, `_leaves` and `_refEdges` answer too. Internal *fields* are excluded by the `internal = true` **option flag**, never by the prefix — the two mechanisms are on different granularities |
