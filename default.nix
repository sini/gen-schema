...
# rule. All four dependencies are root inputs of the ci lock, so every path below is one segment.
#
...
# formulas for one node. Coherent `ci/flake.lock` pins resolve to one store path and `import`
