...
**Dependency class: pure-gen.** gen-schema runs on the pure-gen stack — [gen-prelude](https://github.com/sini/gen-prelude) (the pure utility base), [gen-merge](https://github.com/sini/gen-merge) (the byte-mode module engine that REPLACES `lib.evalModules` + `lib.types`), [gen-algebra](https://github.com/sini/gen-algebra) (the pure `record` algebra), [gen-identity](https://github.com/sini/gen-identity) (the one minting authority) and [gen-graph](https://github.com/sini/gen-graph) (the containment topology — `_topology`, `_roots`, `_leaves` — is its computation). It carries no `nixpkgs.lib` dependency; the constructor takes the five formals `default.nix` declares — `prelude`, `merge`, `algebra`, `identity`, `graph` — each auto-fetched from gen-schema's own lock (pass any explicitly to override). The module-system constructors it exports (identity hashing, strict rejection, validators) are **gen-schema-owned** — they relocated here from gen-algebra on 2026-06-26, leaving gen-algebra fully pure.
...
- **"One construction" is one slot.** A value bound once (a `let`, a formal) merges on all three evaluators. A function reached by a selection written at each call (`computed = fns.c` twice) is a different slot per call: refused on Nix and Determinate, merged on Lix.

#### `mkSchemaEntryType` `mkType` parameter

`mkType` replaces the standard deferredModule merge with a custom entry type constructor. When `null` (the default), kinds produce the standard deferred module with `__functor` wrapping, mixin pipeline, and refinement extraction. When provided, collection extraction still runs first, but `mkType` controls the merged result — the mixin pipeline and `__functor` wrapping are skipped. `options`, `refs` and `refinements` are still derived, and all three come from one evaluation of the option plane an instance imports — the `mkType` result with the kind's `strict`, `keySemantics` and computed fields applied — so `options` lists what an instance of the kind declares, `refs` its reference fields, and a refined option the `mkType` result's module declares is enforced by the instance registry exactly as on the default branch. That evaluation is lazy and shared: one module evaluation per `mkType` kind, forced at the first read of `options`, `refs`, `refinements` or the kind's provenance mark, which reads the same plane. The published kind value applies the `mkType` result's `__functor` to that same module, so a functor that reads its `self` declares one plane at the kind and at every instance. A `mkType` result that is not a module functor publishes its keys beside `options`, which gen-merge's module reader refuses; each of those reads then refuses by name, as an instance of the kind does, naming the module `<gen-schema mkType kind <kind>>` and the first surplus key.
